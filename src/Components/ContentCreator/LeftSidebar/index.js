...
function LeftSidebar({isOpen, user}) {
    ...
    return (
        <div className="flex h-full content-creator-bg">
            {isOpen && (
                <div
                    className="sidebar overflow-auto bg-[#001253] flex-shrink-0 flex flex-col justify-start items-start">
                    {/* Sidebar Content */}
                    <div
                        ...
                        <div className="profile-pic-cover">
                            <img src={user.user.profile_image ? user.user.profile_image : Avatar}
                    ...
                    <div className="w-full flex justify-end flex-col">
                        <ul className="py-4 px-4 w-full">
                            ...
                            >
                                ...
                                {isOpen && <span className="uppercase">{t('navbar.home')}</span>}
                            </li>
                            <div className="mt-6">
                                <AccordionItem
                                    ...
                                    icon="BsGrid"
                                    title={t('admin.leftSidebar.apps').toUpperCase()}
                                    bg="#001253"
                                >
                                    {creatorBlogLinks.map(
                                        (elt) =>
                                            isModuleAllowed(elt.name) && (
                                                ...
                                                >
                                                    {elt.icon}
                                                    {isOpen && <span>{t(elt.name.getTranslationKey())}</span>}
                                                </li>
                                            )
                                    )}
                                </AccordionItem>
                            ...
                            <div className="mt-6">
                                <AccordionItem isDropOpen={getSelectedMenu(location.pathname, creatorProfileLinks)}
                                               icon="BsPersonDash" title={t('admin.leftSidebar.profile').toUpperCase()}
                                               bg="#001253">
